...
class PandasChain:
    ...
    def __commit_block(self,block): 
        ...
        block_hash = block.set_block_hash(self.__id)# Create block hash
...
class Block:
    ...
    def display_header(self): 
        print("sequence ID = %s, status = %s, block hash = %s, previous hash = %s, merkle hash = %s, number of transactions = %s" %(self.__seq_id,
            self.__status, self.__block_hash, self.__prev_hash, self.__merkle_tx_hash, self.get_size()))
    ...
    def add_transaction(self,s,r,v): 
        ...
        new_transaction = pd.DataFrame(data=[[ts, s, r, str(v), tx_hash]], columns=self.__col_names)# Create DataFrame with transaction data (a DataFrame with only 1 row)
    ...
    def set_block_hash(self,hash):
        current_datetime = str(dt.datetime.now())
        # print(self.__prev_hash )

        self.__block_hash = hashlib.sha256(str(str(self.__prev_hash) + hash+ current_datetime + str(self.__seq_id) + self.__merkle_tx_hash).encode('utf-8')).hexdigest()
    ...
    def get_simple_merkle_root(self): 
        self.__merkle_tx_hash = hashlib.sha256(str(''.join(self.__transactions['TxHash'])).encode('utf-8')).hexdigest()
